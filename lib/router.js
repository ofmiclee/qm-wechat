...
router
  ...
  //请求授权 微信回调访问
  .get('/oauth/code', function *() {
    var authCode = this.query.auth_code;
    console.log("=========oAuthCode-queryString============");
    console.log(authCode);

    if (authCode) {
      var appid = yield service.oAuthCode(authCode);
      this.redirect('/#/main/baseinfo/' + appid);
    } else {
      this.body = '授权失败，请稍后重试！'
    }
  })
